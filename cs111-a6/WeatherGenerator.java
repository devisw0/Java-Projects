...
public class WeatherGenerator {
    ...
    int month, double[][] drywet, double[][] wetwet) {
        ...
        double wetwetchance=0;
        double drywetchance=0;
    for(int a=0; a<wetwet.length; a++){
        if ((wetwet[a][0]== longitude) && (wetwet[a][1] == latitude)){
            wetwetchance = wetwet[a][month];
    } 
}
for (int b=0; b<drywet.length; b++){
        if (drywet[b][0]==longitude && drywet[b][1]== latitude){
            drywetchance = drywet[b][month];
    }
}
    double val=0;
for (int c=1; c<d1.length; c++){
    val = StdRandom.uniform();
    if ((d1[c-1]== WET && val <= wetwetchance)){
        d1[c]= WET;
    } else {
       if ( d1[c-1]==WET && val>wetwetchance){
        d1[c]=DRY;
    ...
    if (d1[c-1]==DRY && val<= drywetchance){
        d1[c]=WET;
    } else {
        if (d1[c-1] ==DRY && val> drywetchance){
            d1[c]=DRY;
    ...
    public static int numberOfWetDryDays (int[] forecast, int mode) {
        ...
        int diff= 0;
        for(int d=0;d<forecast.length;d++){
            if(forecast[d]==mode) {
                diff+=1;
            }
        }
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
       ...
       for (int h =0; h<forecast.length;h++){
            ...
            if (forecast[h]!= mode && cnt>=max){                              
                max=cnt;
                cnt=0;
               } else {
                if (forecast[h] != mode && max>=cnt){
                       cnt=0;
                   }
               }
           }
